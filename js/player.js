...
Player = function() {
    ...
    var index = Math.floor(ROT.RNG.getUniform() * pixelEvolution._worldMap.freeTiles.length);
    var randomFreeTile = pixelEvolution._worldMap.freeTiles.splice(index, 1)[0];

    //create a new sprite and put it on that free spot
    Phaser.Sprite.call(this, pixelEvolution._game, randomFreeTile[0]*32, randomFreeTile[1]*32 - 6, 'player');

    this.scale.setTo(0.4, 0.4);

    //enable physics for collision detection and collide to the worlds edge
    pixelEvolution._game.physics.enable(this);
    ...
    this.cursors = pixelEvolution._game.input.keyboard.createCursorKeys();

    //make the camera follow the player
    pixelEvolution._game.camera.follow(this, Phaser.Camera.FOLLOW_TOPDOWN);
};
...
Player.prototype.update = function() {
    if(!pixelEvolution._pause){
        pixelEvolution._game.physics.arcade.collide(this, pixelEvolution._layer);

        for(var i=0; i<pixelEvolution._chests.length;i++){
            var chest = pixelEvolution._chests[i];
